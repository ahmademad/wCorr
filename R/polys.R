...
while(TRUE) {
  ...
  g[[i+1]] <- g[[i]] + qr.solve(qr(J), d[[i]])
  theta <- c(theta[1], g[[i+1]], 0)
  I[[i+1]] <- returnJ(theta)
  J <- I[[i+1]][-c(1,2), -c(1,2)]
  d[[i+1]] <- deltas(x, theta)
  if(sum(g[[i+1]] - g[[i]]) < 0.0000001) {
    break
  }
  i <- i + 1
}
...
returnJ <- function(theta) {
  ...
  for(s in (3:(t_+1))) {
    gamma0[s] <- integrate(gammaToIntegrate(s,0, theta), lower=-Inf, upper=Inf)$value
    gamma1[s] <- integrate(gammaToIntegrate(s,1, theta), lower=-Inf, upper=Inf)$value
    gamma2[s] <- integrate(gammaToIntegrate(s,2, theta), lower=-Inf, upper=Inf)$value
    
    
    xi0[s] <- integrate(xiToIntegrate(s,0, theta), lower=-Inf, upper=Inf)$value
    xi1[s] <- integrate(xiToIntegrate(s,1, theta), lower=--Inf, upper=Inf)$value
    xi2[s] <- integrate(xiToIntegrate(s,2, theta), lower=-Inf, upper=Inf)$value
