...
require(lattice)
require(doBy)
load("Q:/Paul/wCorr/wCorr/vignettes/fast.RData")
...
text(x=c(-2.5,-1.25,0.55,2.3),y=0, labels=paste0("m=",1:4))
theta <- c(-2,-0.5,1.6)
for(i in 1:3) {
...
```{r, echo=FALSE}
biasdf$pcrmse <- sqrt( (biasdf$pc - biasdf$cor0)^2 )
biasdf$psrmse <- sqrt( (biasdf$ps - biasdf$cor0)^2 )
agg <- summaryBy(pcrmse + psrmse + pcb + psb + pct + pst  ~ n + cor0, data=biasdf, FUN=mean, na.rm=TRUE)
pa <- data.frame(t=c(agg$pst.mean, agg$pct.mean),
                 bias=c(agg$psb.mean, agg$pcb.mean),
                 rmse=c(agg$psrmse.mean, agg$pcrmse.mean),
                 cor0=rep(agg$cor0, 2),
                 n=rep(agg$n,2),
                 est=rep(c("Polyserial", "Polychoric"), each=nrow(agg)))
xyplot(bias ~ cor0|est,
      data=pa,
      groups=n,
      type="l",
...
```{r, echo=FALSE}
agg <- summaryBy(pcrmse + psrmse + pcb + psb + pct + pst  ~ n, data=biasdf, FUN=mean, na.rm=TRUE)
pb <- data.frame(t=c(agg$pst.mean, agg$pct.mean),
                 bias=c(agg$psb.mean, agg$pcb.mean),
                 rmse=c(agg$psrmse.mean, agg$pcrmse.mean),
                 n=rep(agg$n,2),
                 est=rep(c("Polyserial", "Polychoric"), each=nrow(agg)))
...
agg <- summaryBy(cor + pst + fpst + psb + pct + fpct + pcb ~ n + cor0, data=fastdf, FUN=mean, na.rm=TRUE)
pe <- data.frame(t=c(agg$pst.mean, agg$fpst.mean, agg$pct.mean, agg$fpct.mean),
                 cor0=rep(agg$cor0, 4),
                 n=rep(agg$n,4),
                 est=rep(c("Polyserial", "Polyserial", "Polychoric", "Polychoric"), each=nrow(agg)),
                 type=rep(c("R", "Rcpp (fast)", "R", "Rcpp (fast)"), each=nrow(agg)))
pe$ntype <- paste(pe$n, pe$type, sep=":")
pe$t <- ifelse(pe$t==0, 0.01,pe$t)

xyplot(t ~ cor0|est,
       data=subset(pe, type=="Rcpp (fast)" & cor0 >= 0),
       groups=n,
       scales=list(y=list(log=10)),
       type="l",
       auto.key=list(lines=TRUE, points=FALSE),
...
This section looks at the agreement between the pure R implementation of the optimizations and the `Rcpp` impelemntation. The code can compute with either option by setting `fast=FALSE` (pure R) or `fast=TRUE` (Rcpp).

This is the summary of all differences between the `fast=TRUE` and `fast=FALSE runs` for the polyserial

```{r polys diff, echo=FALSE}
summary(fastdf$dps)
```

and polychoric

```{r polyc diff, echo=FALSE}
# is there a difference between the fast and slow polychoric?
summary(fastdf$dpc)
```


This plot shows the mean time to compute a single correlation coefficient as a function of `n` size.

```{r, echo=FALSE}
agg <- summaryBy(cor0 + cor + pst + fpst + psb + pct + fpct + pcb ~ n, data=fastdf, FUN=mean, na.rm=TRUE)
pd <- data.frame(t=c(agg$pst.mean, agg$fpst.mean, agg$pct.mean, agg$fpct.mean),
                 cor0=rep(agg$cor0, 4),
                 n=rep(agg$n,4),
                 est=rep(c("Polyserial", "Polyserial", "Polychoric", "Polychoric"), each=nrow(agg)),
                 type=rep(c("R", "Rcpp (fast)", "R", "Rcpp (fast)"), each=nrow(agg)))
