...
where $w_i$ is the weights, $\bar{x}$ is the weighted mean of the ***X*** variable ($\bar{x}=\frac{1}{\sum w_i}\sum_i w_i x_i$), $\bar{y}$ is the weighted mean of the ***Y*** variable ($\bar{y}=\frac{1}{\sum w_i}\sum_i w_i y_i$), and $n$ is the number of elements in ***X*** and ***Y***.^[See the "correlate" function in Stata Corp, Stata Statistical Software: Release 8. College Station, TX: Stata Corp LP, 2003.]
...
To calculate the weighted rank for an individual level is done via two terms. The first term ***A*** is the sum of all weights ***W*** less than or equal to this value of the outcome being ranked ($\boldsymbol{\xi}$)

$$a_j = \sum_i w_i{\rm 1\kern -2.85 pxI}\left( \xi_i < \xi_j \right)$$

where ${\rm 1\kern -2.85 pxI}(\cdot)$ is the indicator function that is one when the condition is true and 0 when the condition is false, $a_j$ is the $j$th value of ***A***, $w_i$ is the $i$th weight and $\xi_i$ and $\xi_j$ are the $i$th and $j$th value of the vector being ranked, respectively. When there are no ties in the ranks the vecor of ranks is simply $a_j + 1$.

When there are ties what is typically done is that each unit recieves the mean rank for all of the tied units. When the weights are all one and there are $n$ tied units the vector of tied ranks would be $\mathbf{v}=\left(a_j+1, a_j+2, \dots, a_j+n \right)$. The mean of this vector is then

$$\frac{1}{n} \left( \sum_i^n a_j i \right)$$
...
$$b_j = \bar{w}_j + \frac{n-1}{2}\bar{w}_j$$

where $\bar{w}_j$  is the mean weight of all of the tied units.

After the ***X*** and ***Y*** vectors are ranked they are plugged into the Pearson correlation coefficient formula shown above, and this Pearson correlation coefficient uses the weights.
...
where $\mathrm{Pr}\left( \rho=r, \theta; Z=z, M=m \right)$ is the probability of the event $\rho=r$ and the cuts points are $\theta$, given the data $z$ and $m$; $\phi(\cdot)$ is the standard normal; and $f(y|z,\rho)$ is the distribution of $y$ conditional on $z$ and $\rho$. Because $y$ and $z$ are jointly normally distributed (by assumption)
...
$$\ell(\rho, \theta;z,m) = \sum_i w_i \ln\left[ \mathrm{Pr}\left( \rho=r, \theta ; Z=z_i, M=m_i \right) \right]$$
...
$$\hat{theta}_{j+2} = Phi^{-1}\left( \frac{\sum_i^N w_i {\rm 1\kern -2.85 pxI}(M_i < j) }{\sum_i^N w_i} \right)$$
...
$$\ell(\rho, \theta, \theta';p,m) = \sum_i w_i \ln\left[\mathrm{Pr}\left( \rho=r, \theta, \theta' ; P=p_i, M=m_i \right) \right] $$
...
$$bias=\frac{1}{n}\sum_i \left(r_i - \rho_i \right)$$
...
This plot shows the RMSE as a function of $n$. The purpose of this plot is not to show an individual value but to show that the estimator is consistent. The plot shows a slope of about $-\frac{1}{2}$, which is consistent with the expected first order convergence for each correlation coefficient under the assumptions of this simulation. 
