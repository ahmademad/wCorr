...
$$a_j = \sum_{i=1}^n w_i{\rm 1\kern -2.85 pxI}\left( \xi_i < \xi_j \right)$$
...
The value of the nuisance paramter $\boldsymbol{\theta}$ is chosen to be

$$\hat{\theta}_{j+2} = \Phi^{-1}(n/N)$$

where $n$ is the number of values to the left of the $j$th cut point ($\theta_{j+2}$ value) and $N$ is the number of data points overall. Here two is added to $j$ to make the indexing of $\theta$ agree with Cox (1974) as noted above. For the weighted cause $n$ is replaced by the sum of the weights to the left of the $j$th cut point and $N$ is replaced by the total weight of all unit

$$\hat{\theta}_{j+2} = \Phi^{-1}\left( \frac{\sum_{i=1}^N w_i {\rm 1\kern -2.85 pxI}(m_i < j) }{\sum_{i=1}^N w_i} \right)$$

where ${\rm 1\kern -2.85 pxI}$ is the indicator function that is one when the condition is true and zero otherwise. 
...
* using a pseudo-RNG, select the the number of bins for ***M*** and ***P*** ($t$ and $t'$) independantly from the set \{2, 3, 4, 5\};^[This means that the simulation uses discrete ordinal variables (***M*** and ***P***) that have 2, 3, 4, or 5 discrete levels. Note that the number of levels in ***M*** and ***P*** are chosen independantly so that one could be 2 while the other is 5 (or any other possible combination).]
...
**`r fig_nums("biasVsRho", display="cite")`.** *Bias vs $\rho$ for Unweighted Correlations.*    
...
**`r fig_nums("rmseVsRho", display="cite")`.** *Root Mean Square Error vs $\rho$ for Unweighted Correlations.*   
...
**`r fig_nums("rmseVsN", display="cite")`.** *Root Mean Square Error vs sample size for Unweighted Correlations.*   
...
**`r fig_nums("rmseVsRho2", display="cite")`.** *Mean Absolute Deviation vs $\rho$ (Weighted).*   
...
**`r fig_nums("rmseVsN2", display="cite")`.** *Root Mean Square Error vs $\rho$ (Polyserial, Pearson, Polychoric panels) or Population Spearman correlation coefficient (Spearman panel) for Weighted Correlations*   
