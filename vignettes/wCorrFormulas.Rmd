...
For the unweighted case the highest rank recieves a value of 1 and the second highest 2, and so on down to the $n$th value. In addition, when data are ranked, ties must be handled in some way. The chosen method is to use the average of all tied ranks. For example, if the second and third rank units are tied then both units would receive a rank of 2.5 (the average of 2 and 3).
...
The term $b_j$ then deals with ties. When there are ties each unit recieves the mean rank for all of the tied units. When the weights are all one and there are $n$ tied units the vector of tied ranks would be $\mathbf{v}=\left(a_j+1, a_j+2, \dots, a_j+n \right)$. The mean of this vector (here called $rank^1$ to indicate it is a specific case of $rank$ when the weights are all one) is then
...
where ${\rm 1\kern -2.85 pxI}$ is the indicator function that is 1 when the condition is true and 0 otherwise. 
...
agg <- aggregate(t ~ n + type, data=ntime, FUN=mean, na.rm=TRUE)
agg$t.mean <- ifelse(agg$t==0, 0.001,agg$t)
...
* For each unit, a uniformly distributed random number was drawn. When that value was less than the probability of inclusion (${\rm Pr}_i$), the unit was included.
...
agg <- aggregate(absdrho ~ rho + usew + type, data=wgt, FUN=mean, na.rm=TRUE)
agg$weight <- ifelse(agg$usew, "Weighted", "Unweighted")

xyplot(absdrho ~ rho|type,
       data=agg,
       groups=weight,
       scales=list(y=list(log=10, cex=0.7), x=list(cex=0.7)),
             type=c("l","g"), 
...
agg <- aggregate(mserho ~ n + usew + type, data=wgt, FUN=mean, na.rm=TRUE)
