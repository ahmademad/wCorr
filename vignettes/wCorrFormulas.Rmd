...
load("Q:/Paul/wCorr/wCorr/vignettes/wgtvrho.RData")
load("Q:/Paul/wCorr/wCorr/vignettes/wgtvn.RData")
```
...
* The root mean square error (RMSE) of the methods as a function of $\rho$ and $n$ in the weighted and unweighed cases
...
Similar to the polyserial, the polychoric is a simple case of two continuous variables ***X*** and ***Y*** that have a bivariate normal distribution. In the case of the polyserial the continuous (latent) variable ***Y*** was observed as a discretized variable ***M***. For the polychoric this is again true but now the continuous (latent) variable ***X*** is observed as a discrete variable ***P*** according to
...
For the weighted case the unweighted result would be consistent if there were not something about the higher weight cases that makes them diffeent from the lower weight cases. Thus, while it is not reasonable to always assume that thee is a difference between the high and low weight cases, boht are possible and it serves as a more robust test of the methods in this package to consider a cases where they are associated. So, when sampling (other than simple random sampling with replacement) is used unweighted correlations may be consistent or may not be. The simulations below expore a case where the unweighted are not expected to be consistent to see if the weighted results are consistent. 

##Weighted simulation study

Simulations are carred out in the same fashion as before with a few extra steps to accomodate weights. The following changes were made:
...
Two simulations were run. The first shows the MAD as a function of $\rho$ and was run for $n=100$ and  $\rho \in \left( -0.99, -0.95, -0.90, -0.85, ..., 0.95, 0.99 \right)$, with 100 iterations run for each value of $\rho$. 
