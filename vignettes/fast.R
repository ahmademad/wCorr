...
for(n in ns) {
  for(cori in cor0) {
    ...
    for(j in 1:reps) {
      ...
      st0 <- system.time(corp <- weightedCorr(x,M, method = c("Polyserial"), weights=rep(1,length(x)), fast=FALSE, ML=FALSE))
      df$pst[ii] <- st0[3]
      df$ps[ii] <- corp
      ...
      st1 <- system.time(polyc <- weightedCorr(M, Q, method = c("Polychoric"), weights=rep(1,length(x)), fast=FALSE, ML=FALSE))
      df$pct[ii] <- st1[3]
      df$pc[ii] <- polyc
