...
require(lattice)
require(doBy)
load("Q:/Paul/wCorr/wCorr/vignettes/ML.RData")
load("Q:/Paul/wCorr/wCorr/vignettes/fast.RData")
load("Q:/Paul/wCorr/wCorr/vignettes/speed.RData")
```
...
where $f(x,y|r)$ is the noramlzied bivariate normal distribution with correlation $\rho$, and  $\boldsymbol{\theta'}$ are the cut points used to discretize ***X*** into ***P***.
...
A simulation was done at each level of the cartesian product of  $\mathtt{fast} \in \{\mathtt{TRUE}, \mathtt{FALSE} \}$, \newline $\rho \in \left( -0.99, -0.95, -0.90, -0.85, ..., 0.95, 0.99 \right)$, and $n \in \{10, 100, 1000\}$. Each iteration was run 100 times. The compulation is run so that the same values of the variables are used for `fast=TRUE` as `fast=FALSE` and then the statistics are compared between the two sets of results.
